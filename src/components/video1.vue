<template>
	<div class="area_video">
		...
		<div class="video_content" v-if="model==3">
			<video id="lbVideo" v-bind:poster="vodList[index].bgUrl" v-bind:src="vodList[index].mp4Url" webkit-playsinline playsinline controls onended="change(this)"></video>
		</div>

		<!-- 直播 -->
		<div class="video_container"  v-if="model==0">
			<video id="liveVideo" v-bind:src="hlsDownstream" webkit-playsinline playsinline autoplay controls></video>
		</div>
...
<!--			<p>如果您听不到直播声音，建议用电脑观看PPT直播</p>-->
		...
		<div class="vod_content" v-if="model==2">
			<video id="recordVideo" v-bind:src="vodName" webkit-playsinline playsinline autoplay controls></video>
		</div>
